# -*- coding: utf-8 -*-
"""
Created on Sat Jan 18 10:33:06 2025
...
for g in gds_files:
    ...
    df_ran = pd.read_csv(path+'full_results.csv', index_col = 0)
    for i in range(len(df_ran)):
        sp = getfeature('sp', df_ran, i, 1e9)
        bp = getfeature('bp', df_ran, i, 1e9)
        sc = getfeature('sc', df_ran, i, 1)
        bc = getfeature('bc', df_ran, i, 1)
        sdc = getfeature('sdc', df_ran, i, 1e3)
        bdc = getfeature('bdc', df_ran, i, 1e3)
        
        ran_file = 'sp'+sp+'_bp'+bp+'_sc'+sc+'_bc'+bc+'_sdc'+sdc+'_bdc'+bdc+'_end'
        ran_file_list.append(ran_file)
    
    """
    ran_file_list = []
        
    if g in ran_file_list:
        print('data already acquired')
    if g not in ran_file_list:
        ...
        def extractfromstring(symbol, filename):
            ...
            if match:
                ...
                return for_fdtd
        def extractfromstringdc(symbol, filename):
            match = re.search(symbol+'(\\d+)_', filename)
            if match:
                number = match.group(1)
                number = int(number)
        ...
        small_dc = extractfromstringdc('sdc', g)
        
        
        big_dc = extractfromstringdc('bdc', g)
        ...
        single_row = [small_pitch, big_pitch, small_count, big_count, small_dc, big_dc, E20, E40, E60, E80]
        len(single_row)
        df_results.loc[len(df_results)] = single_row
        df_results.to_csv(path+'full_results.csv')
